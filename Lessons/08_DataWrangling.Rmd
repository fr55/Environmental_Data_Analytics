...
class(NTL.phys.data.PeterPaul$sampledate) # CSV doesnt keep format info
...
NTL.nutrient.data.PeterPaul <- filter(NTL.nutrient.data, lakename == "Paul Lake" | lakename == "Peter Lake") #filter rows, select colums. | = to or.
...
summary(NTL.nutrient.data.PeterPaul$comments) # the 0 comments are comments that exist for other lakes.
class(NTL.nutrient.data.PeterPaul$comments) # is a factor. doesn not fill it with NAs.
...
summary(NTL.nutrient.data.PeterPaul$lakename) # shows all lakes. there is a droplevel that shows only the lakes in the data set.
...
NTL.nutrient.data.PeterPaul.skinny <- select(NTL.nutrient.data.PeterPaul, 
                                             lakename, daynum, year4, sampledate, depth:po4)   # to remove we put a - sign (only for select). filter is different
```
# up to here is data wrangling
...
NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.skinny, "nutrient", "concentration", tn_ug:po4) # function gather and spread. we define new columns and gather existing ones
count(NTL.nutrient.data.PeterPaul.gathered, nutrient). # gets the bigger number and fills with NAs
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) # eliminates the NAs
...
# Spread nutrient data into separate columns   # opposite of gather
NTL.nutrient.data.PeterPaul.spread <- spread(NTL.nutrient.data.PeterPaul.gathered, nutrient, concentration) # two values missing. must be 2 rows with only NAs.
...
interval(first(NTL.phys.nutrient.data.PeterPaul$sampledate), last(NTL.phys.nutrient.data.PeterPaul$sampledate)) #more reprodusable
```
