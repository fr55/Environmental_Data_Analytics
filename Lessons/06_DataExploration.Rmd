...
#head(USGS.flow.data) # not useful for kniting it (looks at six rows)
class(USGS.flow.data$datetime) # it is a factor. Problem. Formating dates
...
```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #remove the entire row  (look for how to removes NAs from a column)
dim(USGS.flow.data)
...
```{r, fig.height = 3, fig.width = 4}
#three frecuency poligons for diff data. we dont want to mix variables with diff units.
ggplot(USGS.flow.data.complete) +
