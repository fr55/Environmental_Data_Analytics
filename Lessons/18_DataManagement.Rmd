...
[Climate](https://www.ncdc.noaa.gov/cdo-web/)

[State and County Emission Summaries](https://www3.epa.gov/air/emissions/where.htm)
...
# Coerce into a data frame, ensure consistent lengths FRA:get rid of extra data.
...
riverdata$Name <- str_replace(riverdata$Name, "\\[.*\\]", "") #double\\ means any spaces.
...
class(riverdata$Length.km) # is a character.Remove commas
...
ggplot(riverdata, aes(x = Length.km, y = Discharge.m3s, fill = DrainageArea.km2)) + 
  geom_label(data = subset(riverdata, Name == "Yukon" | Name == "Syr Darya–Naryn"), 
  ...
  theme_classic() + 
  scale_y_log10() + #because ther was an outlier
