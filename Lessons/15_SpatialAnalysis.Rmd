...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  ...
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   ...
                   stroke = FALSE, 
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`)) # RA: what field you want to pop up when you click the point
#Show the map
myMap
```
...
* Visualizing all the different projections [link](https://map-projections.net/singleview.php) #BUENO!!!
...
EPAair_PM_avg_sf_UTM <- st_transform(EPAair_PM_avg_sf, crs=26917) # there is a web page to look the number crs (spatial reference search)
