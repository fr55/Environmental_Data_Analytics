...
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     ...
                     random = ~1|Week)
TempTest.auto
# what the variability is among the weeks. That is why it gives a StdDev. (the random effect). Fixed effects gives coef.
...
TempTest.mixed <- lme(data = PeterPaul.summertemp,
                     ...
                     #sampledate is duplicated in some cases, so need to split up by lake. FRA: This is only beacuse R doesnt like  that the sample were taken the same time.
                     correlation = corAR1(form = ~ sampledate/lakename|Week, value = 0.423), # we put manually the 0.423
                     #define method as restricted maximum likelihood
                     method = "REML")
summary(TempTest.mixed)

#Results: Random we got a stddev. Fixed effects: lakenamePeter Lake coef --> no diff between lakes. The interaction is also not significant.
...
# why did we wanto to use random effect? --> We now there is variability. why dont use that for our advantage.
