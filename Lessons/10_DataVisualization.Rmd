...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic() #Kateri like this one (no grid)
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #you can put any name
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #one way of doing it
print(O3plot5)  

theme_set(mytheme) #this is other way of doing it. Better for several plots.
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #this theme only changes the things you specify
print(O3plot7)  
...
```{r, warning = FALSE}
#install.packages("viridis")    #####use always in the beginign
...
display.brewer.all(n = 9) # from the website you cna use. color blind!!!!
...
NvsP2 <-
  ...
  xlab(expression("Total Phosphorus, (\U003BCg/lt)")) + #there are codes for greek leters
  ylab(expression("Total Nitrogen (\U003BCg/lt)")) +
...
print(NvsP2)  #change legend titles (we tried to do it)
...
# place your additional edits here
  ...
  scale_color_manual(values = c("#7fcdbb", "#41b6c4", "#1d91c0", "#225ea8", "#0c2c84")) + #from the website
  #scale_color_viridis(discrete = TRUE) + #another option. problem with the yellow.
  theme(legend.position = "right") +
  ylab(expression("Concentration (\U003BCg/lt)")) +
  xlab(expression(""))
