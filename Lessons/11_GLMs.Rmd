...
PeterPaul.nutrients <- filter(PeterPaul.nutrients, depth_id > 0) #is a weird ID that was given. Just remove for this example. 

# set depth_id to factor
PeterPaul.nutrients$depth_id <- as.factor(PeterPaul.nutrients$depth_id) # we want later factor instead of numeric.
...
Are Ozone levels below the threshold for "good" AQI index (0-50)?  #T test.

```{r}
summary(EPAair$Ozone)

# Evaluate assumption of normal distribution
shapiro.test(EPAair$Ozone) #test aprox. to normal dist. pvalue less than 5% not Ok. So lets take a look
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram(stat = "count") #wrong peak and long tail to the right
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone) # other way to look at it. We go ahead any way becuase a lot of data and shape resembles

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") # we are asking if data below threshold than 50.
O3.onesample # store the result. P value low. reject null hypothesis. give also t score. Linear model y = 36.9 ] error
```

What information does the output give us? How might we report this information in a report?

> ANSWER: Ozone AQI values in 2017-2018 were significanly lower than 50 (one sample t-test; t = -41.9, df =184, p<0.0001) 

### Two-sample t-test #second alpha value in the linear model
...
var.test(EPAair$Ozone ~ EPAair$Site.Name) # f statistic and pvalue. variance the same? Pvalue low, the variance is not the same. The interval does not overlap 1.

ggplot(EPAair, aes(x = Ozone, color = Site.Name)) +
  geom_freqpoly(stat = "count") # just to look how they relate to each other (look at tails, peaks)
# two ways of doing it
# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Site.Name)
O3.twosample
O3.twosample$p.value # we have a significance differnet in the means between the 2 groups.
# so the linear model is y = Blackstone(38.48) + BrysonCity(35.18) + error
# Format as a GLM #other way of doing it
O3.twosample2 <- lm(EPAair$Ozone ~ EPAair$Site.Name)
summary(O3.twosample2) #diff pvalue. same estimates but bryson is relative. We get a lot of aditional info not useful right now.
```
### Non-parametric equivalent of t-test: Wilcoxon test #for not normal distribution. distribution free
...
O3.twosample.wilcox # much more limited models but we get the info that we wanted. dont tell us the prediction for the means. less explanatory power.
```
...
ggplot(EPAair, aes(x = PM2.5, color = Site.Name)) +
  geom_freqpoly(stat = "count") # fairly good bell curve. not crazy tails. maybe we may wanna proceed with anova
qqnorm(EPAair$PM2.5); qqline(EPAair$PM2.5) # looks good except for the tails. not surprising for environ data.

bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) # var equal? here they are not significance diff. reach assumption of equal differnece.
#two ways
# Format as a GLM
PM2.5.anova <- lm(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova) # linear model y = 36.7261 - 4.4266Bryson -3.24TripleOak #here we only know that the intercept is signif diff from 0, and the other variables signif diff from intercept.

# Format as an aov
PM2.5.anova2 <- aov(EPAair$PM2.5 ~ EPAair$Site.Name)
summary(PM2.5.anova2) #just give us if site is significant predictor. we need a posthoc test

# Run a post-hoc test for pairwise differences
TukeyHSD(PM2.5.anova2) #there is no diff between Triple and Bryson
...
print(PM2.5.anova.plot) #to look at the distribution
```
What information does the output give us? How might we report this information in a report?

> ANSWer: (ANOVA; F = 16.16, df =1898,  p < 0.0001). Our average pm25 values where signif....     .On the plot. Put a star on blackstone or group them (bryson and triple under a hay or something geomtext or something)
...
PM2.5.kw # doesnt indicate which groups are diff.
#the next tells you but we are not going to do it.
...
## TWO-WAY ANOVA #convination of multiple diff outputs y = alphaa1 + alphaa2+ alphab1 + alphab2 + error
### Main effects
...
```{r}
TNanova.main <- lm(PeterPaul.nutrients$tn_ug ~ PeterPaul.nutrients$lakename + # the plus is the main effefct anova
                     PeterPaul.nutrients$depth_id)
summary(TNanova.main) #TN = y = 309(paul lake dpeth id of 1) + 105.2PeterLake(id1) + 97id2.. + error. id5 and id6 dont have relevant pvalues.
#two ways to do it
...
TukeyHSD(TNanova.main2) # all the combinations

# Plot the results
# How might you edit this graph to make it attractive?
# How might you illustrate significant differences? #using diff pallete
...
```{r}
TNanova.interaction <- aov(PeterPaul.nutrients$tn_ug ~ PeterPaul.nutrients$lakename * #interaction 
                             PeterPaul.nutrients$depth_id)
...
```{r}
TukeyHSD(TNanova.interaction) #we have an interaction so we oook the third level. explained below
```
