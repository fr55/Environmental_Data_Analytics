...
## SIMPLE AND MULTIPLE LINEAR REGRESSION (line of best fit, continous data)
...
> ANSWER: significant negative correlation between irradiance and depth (lower light levels at greater depths), and that this model explains about 31 % of the total variance in irradiance (linear regression, R2 = 0.31, df = 15449, p < 0.0001(the depth p value in this case)). We dont use the Fstatistic. We use it only for ANOVA. If is part of your question you can report the pvalue of the intersept.
...
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) # we remove the 0 to be able to log  transform. they are only 3 values. not so concerned on loosing the data. to few.
...
#you can change the transparency (alpha). or an anotation layer.
```
...
summary(TPregression) #TP = 6 (0ox and 0 depth, not real) +1.5(depth) + 0.94(DO) + error
...
TPmodel <- lm(data = PeterPaul.naomit, tp_ug ~ temperature_C + tn_ug) # you run the best fit model on its own.
...
summary(TNancova.main) #TN = 353(paul lake depth 0) + 135.361(peter diff) -9.7depth(not significant). very little variance.

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction) # TN = 325 + 185(Peter) + 20(if paul)depth - 48(if peter)depth + error. Slightly more variance than the last one.

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + #color in aes two lines + 
  geom_point() + 
