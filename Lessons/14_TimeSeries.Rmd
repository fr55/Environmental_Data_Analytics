...
#z = 4.3966, n = 132, p-value = 1.099e-05              Low pvalue. there is a trend over time. z positive. positive trend over time.
...
USGS.flow.data.interpolated <- approx(USGS.flow.data.trimmed$datetime,
                                      ...
                                      method = "linear", n = 3630) # so you dont generate new points. I think this was to fill rows with NAs. It give us a list.
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1) ,frequency = 12) #12 beacuse is monthly. you can out 365.
...
summary(USGS.smktest) #gives as each month. the trends each season. Season 11 biggest trend. No trend in season 5 for example
```
